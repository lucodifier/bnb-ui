...
import { aplicarMascara, retornaPlaceHolder, validarChave } from "../../../services/pix-com-chave.service";
import { obterTransacoesAnterioresPorChave } from '../../../services/transferencia.service'
import { initialFormTransferenciaAnterior, TransferenciaAnteriorModel } from "../../models/TransferenciaAnterior.model";
...
const MSG_ERROR_CONSULTA = 'Transação não concluída. Falha de comunicação para consulta de dados do recebedor. Tente novamente mais tarde';
const MSG_ERROR_FAVORITOS = 'Ocorreu um erro ao recuperar os favoritos. Tente novamente mais tarde.';
const MSG_ERROR_TRANSFERENCIAS = 'Ocorreu um erro ao recuperar as transações anteriores. Tente novamente mais tarde.';
...
export function PixComChave() {
  ...
  const [transferenciasList, setTransferenciasList] = useState(initialFormTransferencias);
  ...
  const handleTabIndexChange = (event: any, value: any) => {
    if (value === 1) {
      setChavePixList(favoritosList.map((item: FavoritoModel) => {
      ...
      setChavePixList(transferenciasList.map((item: TransferenciaAnteriorModel) => {
        return new InfoChavePix(
          item.nm_pri_ttl_dst,
  ...
  const obterFavoritos = (bancosResgatados: Array<BancoModel>) => {
    obterFavoritosPorCpfCnpj(dadosSessao.codigoCliente).then((response: any) => {
  ...
  const obterTransferenciasAnteriores = () => {
    ...
    obterTransacoesAnterioresPorChave(
      ...
      const transferencias: Array<TransferenciaAnteriorModel> = response.data.result;
  ...
  const confirmarConsulta = (chave: string) => {
    ...
    consultarChave(
      ...
      dadosSessao.codigoCliente).then((response: any) => {

        console.log(response.data);
        if (response.status === 200 && response.data.result.codigo === '0') {
          setChaveConsultada(response.data.result.dadosChavesEnderecamento/*aplicarMascaraSeCpf(response.data.result.dadosChavesEnderecamento)*/);
          setOpen(true);
        } else {
          setSnack({
            open: true,
  ...
  return (
    <Grid container spacing={3} classes={{ root: classes.main }}>
      <Grid item lg={12} xs={12}>
        ...
        {isDesktop && <Grid style={{ display: 'flex' }}>
        ...
        {!isDesktop && <Typography style={{ fontWeight: 'bold', color: '#a6193c' }}>
          Pagar com Chave Pix
        </Typography>}
        ...
        <Grid item xl={6} lg={6} md={6} sm={12} xs={12}>
          <div className={classes.buttonGroup}>
            <SquareButton label="CPF/CNPJ" icon="cpfCnpj" color="primary" customOnClick={() => handleTipoChaveChange(TipoChaveEnum.CPF_CNPJ)} selected={tipoChavePix == TipoChaveEnum.CPF_CNPJ} />
            <SquareButton label="Celular" icon="celular" color="primary" customOnClick={() => handleTipoChaveChange(TipoChaveEnum.PHONE)} selected={tipoChavePix == TipoChaveEnum.PHONE} />
            <SquareButton label="E-mail" icon="email" color="primary" customOnClick={() => handleTipoChaveChange(TipoChaveEnum.EMAIL)} selected={tipoChavePix == TipoChaveEnum.EMAIL} />
            <SquareButton label="Chave aleatória" icon="evp" color="primary" style={{ marginRight: '0px' }} customOnClick={() => handleTipoChaveChange(TipoChaveEnum.EVP)} selected={tipoChavePix == TipoChaveEnum.EVP} />
          </div>
        </Grid>
      </Grid>
      ...
      <Grid item lg={12} xs={12} >
        <AppBar position="static" classes={{ root: classes.appBarStyle }}>
          <Tabs
            ...
            classes={{
              indicator: tabIndex == 1 ? classes.indicatorFavoritos : classes.indicatorTransferencias,
      ...
      <Grid container justifyContent="flex-end" alignItems="center">
        ...
        <IconButton style={{ marginRight: '-10px' }} disabled={currentPage === 0} onClick={() => setCurrentPage(currentPage - 1)}>
          <ArrowBackIos classes={{ root: classes.pageableIcon }} />
        </IconButton>

        <IconButton disabled={currentPage + 1 === pages} onClick={() => setCurrentPage(currentPage + 1)}>
          <ArrowForwardIos classes={{ root: classes.pageableIcon }} />
        </IconButton>
      </Grid>

      <SimpleDialog open={open} title="Confirma o destinatário?" handleClose={() => handleDialogClose()} >
        <CustomUserCard nomeRecebedor={chaveConsultada.nome}
      ...
      <Snackbar
        ...
        <Alert variant="filled" onClose={handleSnackClose} severity={snack.severity} style={{ width: '90%', backgroundColor: "#ebae2a" }}>
          {snack.message}
        </Alert>
